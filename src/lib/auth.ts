    ...
    providers: [
        CredentialsProvider({
            ...
            async authorize(credentials) {
                if (!credentials?.userHandle || !credentials?.challengeId || !credentials?.authResponse) {
                    return null;
                }
                ...
                try {
                    const rpID = process.env.WEBAUTHN_RP_ID || 'veridex.network';
                ...
                console.log('[Auth] WebAuthn verified for wallet:', walletAddress);
